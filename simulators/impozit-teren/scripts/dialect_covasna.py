"""Covasna, out of the volume it shares with its neighbour. See cnp_brasov.py for the reading.
...
def _names() -> dict[str, str]:
    ...
    path = ROOT / "data" / "fond-funciar-cv-2014.json"
    if not path.exists():
        return {}
...
def parse(name: str, is_local) -> tuple[list[dict], list[dict], list[str]]:
    known = _names()
    return parse_county(name, is_local, "CV", "COVASNA", known.get)
