...
`TERITORIAL - SERVICIU PUBLIC DECONCENTRAT`. In 2025 that is **601 rows**.

Each row is assigned to a **family** by an explicit prefix table in
`scripts/build_deconcentrare.py` — not by fuzzy matching, because the source writes the same
service several ways (`DIRECȚIA SANITAR-VETERINARĂ`, `... SANITAR -VETERINARĂ`,
`... SANITAR- VETERINARĂ`). A fuzzy matcher would fix those and quietly merge others. Every name
that matches no prefix is written to `unmatchedNames` and counted in `summary.unmatched`; the
build fails its test if that count is ever non-zero, so a source refresh that adds a family
surfaces instead of dropping rows.
...
- **regional** — one office per county today, proposed one per development region. These are the
  549 offices the headline reduces to 117.
...
```
601  deconcentrated offices in the source
−  9  municipal (Bucharest, sectors)          -> municipalNames
= 592 matched to a family                     -> summary.matchedOffices
      549 in the 16 regional families         -> reduced to 117
       43 already regional or special
```
...
- **ANFP's scope is public-function institutions.** Contractual-staff services are
  under-represented (14 environment agencies, one consumer-protection commissariat). The true
  reduction is larger, not smaller.
