...
**Brașov and Covasna took three attempts, and the second one is the instructive failure.**

The first verdict was "4 land tables for 103 localities" — the same table count that had already
been wrong about Suceava. This study puts land inside annexes titled after buildings, so the
count found four tables in a volume that prices every locality in two counties.

The second attempt **reported 91,4% coverage and published impossible numbers**: Brașov's zone B
dearer than its zone A, 5 500 against 1 500, where the page says **1.150 / 980 / 800 / 630**. Two
generic mistakes underneath it — the town reader accepted any table whose header carried `ZONA`,
and the building tables carry it too; and the rural reader took the first numeric cell of a row
as its price, where the first cell is the **row number**, so `| 1 | Comunele Hărman… | 242,0 |`
came out as 1,0. Coverage was 91% throughout. It was caught only because the zone prices were
printed side by side and B cannot exceed A.

The third attempt is verified against the page, county by county. **Both read at 100%.**

Two more bugs on the way, both silent:

- **The rural reader had to be restricted to the building-land table.** The agricultural table
  on the same page carries the same row labels — the circumscriptions are identical — so
  reading both mixed 17,5 and 23,6 in among 242 and 131.
- **A commune's name was being cut in half.** The helper stripped the first word of the
  register's name to remove a rank prefix, and communes without one lost their first word:
  `VAMA BUZAULUI` became `BUZAULUI`, `POIANA MARULUI` became `MARULUI`. Four communes in Brașov
  and five in Covasna were published under half their own names, which the roster gate caught
  and the coverage figure did not. Fixing it took Brașov from 93,1% to 100% and Covasna from
  88,9% — below this repository's bar, and correctly refused — to 100%.

Brașov prices its countryside **by court circumscription**, seat and villages apart, with the
ring around the city named out at nearly twice the rest: Hărman, Sânpetru and Cristian at 242,0
against 131,0. Which commune sits in which circumscription is not in the study, because the
decision that says so is law — HG 1217/2023 — and this repository already carries it in the
justice simulator's `arondare-2023.json`. Covasna prices in three columns instead: a named list,
then all other commune seats, then all other villages.

**Brașov was predicted at 13,93 mld EUR and reads at 11,91 — 0,86×.** Seventh out-of-sample
check, seventh pass.

**Suceava and Botoșani were written off here, and should not have been.** The verdict was
"42 land tables for 114 localities, ceiling 37%", reached by counting tables — the same
measurement that would have discarded Ialomița, whose study contains **no land table at all**
and reads at 100% because its prices are prose. Counting tables asks how a document is
typeset. The question that matters is whether it says which localities each price applies to.

Suceava says so twice over. Its countryside is priced in buckets that **name their own
members**, with a catch-all: `MEDIUL RURAL 1 (piața specifică pentru localitățile: ȘCHEIA,
BOSANCI, …)` and `MEDIUL RURAL 2 (excepție localitățile: …)`. One list and its complement cover
the circumscription by construction. And the study publishes **its own circumscription
rosters**, numbered under Municipii / Orașe / Comune, citing HG 1217/2023 — so the complement
resolves without leaving the document.

**93,9% for Suceava and 93,6% for Botoșani, on one reader with no changes between them.**

Five things had to be got right, and the shape of each is worth keeping:

- **Three circumscriptions split their countryside in two and three do not.** Fălticeni,
  Câmpulung, Vatra Dornei and Gura Humorului have a single unnumbered `MEDIUL RURAL` section
  with no list, which *is* the whole of that circumscription. Requiring a list left 44 communes
  — four entire circumscriptions — unpriced at 54%.
- **The rosters print two columns to a line.** `1 Arbore 15 Iaslovăț` read as one entry gives
  `Arboreiaslovat`, which is not a commune.
- **The headings are letter-spaced for emphasis** — `SUCEA V A`, `V ATRA DORNEI` — so a name
  read literally matches nothing.
- **The smaller towns are never introduced.** Solca, Cajvana, Vicovu de Sus and five others
  carry their name as a bare running header, and are priced by category rather than by zone.
  Reading only the zone grid left eleven of sixteen towns unpriced.
- **The word "circumscripția" is unreliable** — spelled `CICIRCUMSCRIPȚIA` once and not on its
  own line twice. What every roster page shares is the sentence citing the decision.

**The first out-of-sample failure, and it is real.** Botoșani was predicted at 4,47 mld EUR and
reads at **2,34** — 0,52×, outside the stated 1,61×. The reader was checked against the source
before that was accepted: all seven of its towns match to the leu, Botoșani town at 317 lei/m²
in zone A against Iași's 600 **euro**. Botoșani is simply cheaper than the size of its county
seat predicts, and it stays in the fit — the test for holding a county out is a *mechanism*,
as Ilfov has, not that it makes the number worse.

Reading them cost the model a little and that is left visible: leave-one-out **1,58× → 1,61×**,
R² **0,77 → 0,74**. Thirty counties is a harder thing to fit than twenty-eight, which is what
finding out looks like.
...
is layout, not method, which is why there are still four readers and not one with four modes.

**Its assignment is a table carrying both levels at once**, and a village may sit in a
different category from its own commune — Frasinu is II inside a commune that is I. **100%,
3 towns, 51 communes, 162 villages.**

**Extravilan is priced by what the land could become.** Three figures per locality under
criteria rather than soil: *posibil de transferat în intravilan* at 49 100 EUR/ha, *situat în
planul II* at 25 600, *destinaţie exclusiv agricolă* at **5 750**. Nine times between the ends,
and the last is the one taken — the hectares priced here are the register's agricultural ones,
and land that might become building land is a development option, not a field. A second table
covers the fifty-one communes the first ignores, county-wide by position: 6 560 at a locality's
edge, **5 300 in the second plane**, 2 920 unproductive. Worth naming rather than smoothing:
the chamber's own unproductive figure is 10% above what its own coefficient gives from 5 300.

**One bug here was invisible by construction.** The two circumscriptions price the same three
categories differently — 8,6 against 11,1 EUR/m² for category I — and the assignment tables sit
pages before the prices that apply to them. Merging them into one dictionary priced every
village in the county from whichever circumscription was read first: a third of a county
silently wrong, with every name matched and every number real. Assignments are now closed
against the prices of their own circumscription, in page order.
...
|---|---|---|
| ~~Suceava~~ | SV, BT | **wrong — both read, 93,9% and 93,6%. The 37% came from counting tables** |
| ~~Brașov~~ | BV, CV | **wrong — both read at 100%. The 4% came from counting tables** |
...
**Two of those rows were wrong, and the method that produced them is discredited.** Counting
land tables measures typesetting, not content: Ialomița has none and reads at 100%, Suceava was
called 37% and reads at 94%. Craiova, Pitești, Galați and Timișoara were each checked a
different way — a scan has no text at all, and Craiova's own index prices 23 localities of 359
— so those stand. **Brașov does not**, and it is marked unverified rather than quietly kept:
it was dismissed by the same table count and has not been re-opened.

The remaining twelve counties are estimated because nothing better has been read yet, not
because it has been established that nothing better exists.
...
Thirty-two counties read: **2 289 localities, 17,2 M ha, 72,0% of Romania, 267 mld EUR.**
